 ...
 *
 * This repository lints Markdown only, so the sole cache type is the npm
 * cache used by markdownlint-cli2:
 *
 *     npm-markdownlint-{OS}-node-{version}-mdlint-{version}
 *
 * The script keeps only the most recently accessed cache per group
 * (npm-markdownlint-{OS}) and deletes everything older. Set INPUT_DRY_RUN
 * to "true" to log what would be deleted without deleting.
 */
...
module.exports = async ({ github, context, core }) => {
    ...
    try {
        ...
        for (const cache of caches) {
            let prefix;
            if (cache.key.startsWith("npm-markdownlint-")) {
                // Group by npm-markdownlint-{OS} (e.g., npm-markdownlint-Linux)
                const parts = cache.key.split("-");
                prefix = `${parts[0]}-${parts[1]}-${parts[2]}`;
            } else {
                // Unknown cache type: use the full key as the prefix so it
                // is never grouped with (and thus never deletes) another key.
                prefix = cache.key;
            }
